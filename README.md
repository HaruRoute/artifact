...
> **서버 운영 종료 (2026.06.30)** — AWS EC2 인스턴스 및 RDS를 종료하여 현재 접속 불가합니다.

| 환경 | URL |
|------|-----|
| 서비스 (k8s) | http://44.203.66.174 (서버 종료) |
| Jenkins CI/CD | http://54.172.210.38:8080 (서버 종료) |
...
Jenkins EC2 (54.172.210.38:8080)
    ...
    │ 3. Test          - JUnit 단위 테스트 (mvn test, 실패 시 빌드 중단)
    │ 4. Build         - Docker 이미지 빌드 (docker compose build)
    │ 5. Push to ECR   - ECR에 :빌드번호 + :latest 태그로 push
    │ 6. Deploy to k3s - SSH → k3s EC2에 kubectl apply + rollout restart
    │ 7. Health Check  - 프론트/백엔드 HTTP 상태 확인
    ▼
...
| 파일 | 내용 |
|------|------|
| `backend-deployment.yaml` | Spring Boot Deployment + Service, CPU 리소스 + Startup/Readiness/Liveness Probe, 60s terminationGracePeriod |
| `backend-hpa.yaml` | HPA (CPU 70% 초과 시 pod 2→4 자동 스케일 아웃, `autoscaling/v2`) |
| `backend-pdb.yaml` | PodDisruptionBudget (minAvailable: 1, 롤링 업데이트 중 최소 1 pod 보장) |
| `frontend-deployment.yaml` | nginx Deployment + NodePort Service |
| `ai-server-deployment.yaml` | FastAPI Deployment + Service |
| `redis-deployment.yaml` | Redis Deployment + PVC(1Gi) + RDB 스냅샷(60s) 영속성 |
| `rate-limit-middleware.yaml` | Traefik Middleware (IP당 평균 100 req/s, 순간 최대 200 req) |
| `ingress.yaml` | Traefik Ingress (/ → frontend, /api → backend) + rate-limit 미들웨어 적용 |
...
| 기술 | 역할 |
|------|------|
| Jenkins | CI/CD 파이프라인 (JUnit 품질 게이트 → 빌드 → ECR push → 배포 자동화) |
| Docker / Docker Compose | 컨테이너 빌드 및 로컬 실행 |
| Amazon ECR | Docker 이미지 레지스트리 (빌드번호 태그로 버전 롤백 지원) |
| k3s (Kubernetes) | 경량 쿠버네티스 클러스터 |
| Amazon RDS MySQL 8.0 | 관리형 데이터베이스 |
| Amazon EC2 | 서버 인프라 (Jenkins, k3s) |
| Traefik Ingress | k8s 경로 기반 라우팅 + Rate Limiting 미들웨어 |
| Redis | API 응답 캐싱 (TTL 기반) + RDB 영속성 (PVC 마운트) |
| Prometheus | 메트릭 수집 (Spring Boot Actuator + Node Exporter) |
| Grafana | 메트릭 시각화 + Discord 알람 (CPU 80%, HTTP 5xx 5%, JVM heap 85%) |
| k6 | 부하 테스트 (Stress 3라운드 + Soak 1라운드, 총 1,000VU 검증) |
| Resilience4j | Circuit Breaker (AI 서버 장애 격리, fallback 응답) |
| JUnit / Mockito | 백엔드 단위 테스트 (20개 클래스, Jenkins 파이프라인 품질 게이트) |
...
| 단계 | 적용 변경사항 | error_rate | spots p95 | route p95 | http p95 |
|------|-------------|-----------|-----------|-----------|----------|
...
#### Phase 5: 안정성 강화 — Graceful Shutdown · Probes · Circuit Breaker · PDB · Rate Limiting · Redis 영속성 · Grafana 알람

**적용 항목 및 목적**

| 항목 | 설정 | 목적 |
|------|------|------|
| Graceful Shutdown | `server.shutdown=graceful`, `timeout-per-shutdown-phase: 30s` | 롤링 업데이트 중 처리 중인 요청 완료 후 종료 |
| Startup Probe | `/api/actuator/health/liveness`, failureThreshold 20 × 5s = 100s 대기 | JVM 워밍업 완료 전 트래픽 유입 차단 |
| Readiness Probe | `/api/actuator/health/readiness`, 10s 간격 | 준비 안 된 pod를 로드밸런서에서 자동 제외 |
| Liveness Probe | `/api/actuator/health/liveness`, 15s 간격 | 데드락·무한루프 pod 자동 재시작 |
| Circuit Breaker | Resilience4j, 실패율 50% 초과 시 30s open, half-open 3회 검증 | AI 서버 장애 시 fallback 응답 즉시 반환, 연쇄 장애 차단 |
| PodDisruptionBudget | `minAvailable: 1` | 노드 점검·스케일다운 중 최소 1 pod 보장 |
| Rate Limiting | Traefik Middleware, IP당 평균 100 req/s · burst 200 | DDoS·트래픽 폭주로부터 백엔드 보호 |
| Redis PVC | 1Gi local-path, RDB 스냅샷 60s 간격 | Redis 재시작 후 캐시 데이터 복구, 콜드 스타트 방지 |
| Grafana Discord 알람 | CPU 80%, HTTP 5xx 5%, JVM heap 85% 초과 시 Discord 채널 알림 | 장애 조기 감지 및 즉각 대응 |
| Jenkins JUnit 게이트 | `mvn test -Dtest=!BackendApplicationTests` (Docker Maven 컨테이너) | 테스트 실패 시 빌드·배포 자동 차단 |
...
| 테스트 | ✅ JUnit 단위 (20개 클래스, Jenkins 품질 게이트) + Playwright E2E | JaCoCo 커버리지 측정, SonarCloud 정적 분석 연동 |
| 부하 테스트 | ✅ k6 4라운드 완료 (Stress × 3 + Soak × 1) | HPA 적용 완료, 캐시 히트율이 1000VU 성능의 핵심 인자로 확인 |
| 안정성 | ✅ Graceful Shutdown, Probes, Circuit Breaker, PDB, Rate Limiting, Redis 영속성, Grafana Discord 알람 | SonarCloud 코드 품질 게이트 추가 |
...
### 홈 화면
![홈 화면](images/screen_home.png)

- Kakao Map 기반 서울 관광지 마커 전체 표시
- 좌측 경로 플래너 사이드바: 이동수단 선택, 총 거리·요금 실시간 확인
- 우측 통합 사이드바: AI 챗봇 위젯 / 즐겨찾기 / 저장 계획 탭

### 지도 관광지 검색
![지도 검색](images/screen_map.png)

- 시도·시군구·관광 타입·관광지명 복합 필터 검색
- "이 위치에서 검색" 플로팅 버튼으로 현재 지도 영역 기준 관광지 즉시 갱신
- 마커 클릭 → 팝업에서 즐겨찾기 등록 및 경로 추가

### AI 챗봇 위젯
![챗봇 위젯](images/screen_chatbot.png)

- 우측 하단 FAB 버튼으로 접근하는 플로팅 팝업
- RAG 기반 관광지 추천 + 마크다운 렌더링
- AI 응답 내 장소명을 인라인 칩으로 렌더링 → 클릭 시 경로 플래너 자동 추가
- 빠른 질문 버튼 5개, 멀티턴 대화 컨텍스트 유지

### 게시판
![게시판](images/screen_board.png)

- 전체 / 공지 / 자유 / 질문 4탭 구조
- 전체 탭: 최신 공지 바 + 자유 게시판 조회수 상위 3개 그리드
- 자유 게시글에 저장된 여행 루트 첨부 가능

### 공지사항
![공지사항](images/screen_notice.png)

- 관리자(ADMIN)만 작성·수정·삭제 가능
- is_pinned 설정 시 게시판 상단에 고정 표시

### 게시글 상세
![게시글 상세](images/screen_post.png)

- 첨부된 여행 루트 미리보기 (첫 번째 장소 커버 이미지 자동 추출)
- "내 루트로 가져오기" 버튼으로 경로 플래너에 즉시 로드 (로그인 필요)

### 내 보관함
![내 보관함](images/screen_library.png)

- 즐겨찾기 관광지 목록 관리
- 저장된 여행 계획 목록 조회·불러오기·수정·삭제

---

## 14. 시연 시나리오

### 시나리오 1: AI 챗봇으로 여행 코스 추천 → 경로 플래너 연동

1. 우측 하단 챗봇 FAB 버튼 클릭
2. "서울 주말 데이트 코스 추천해줘" 입력
3. AI가 RAG 기반으로 관광지 3~5개 추천 (인라인 장소 칩으로 표시)
4. 장소 칩 클릭 → 경로 플래너에 자동 추가
5. "동선 최적화" → 이동 수단 선택 (자동차 / 대중교통 / 도보)
6. 총 거리 및 이동 요금 확인 후 "계획 저장"

### 시나리오 2: 지도에서 관광지 탐색 → 즐겨찾기 등록 → 경로 구성

1. 시도: 서울, 시군구: 종로구, 관광 타입: 문화시설 필터 선택 후 검색
2. 지도 마커 클릭 → 관광지 팝업에서 ★ 즐겨찾기 등록
3. "이 위치에서 검색" 버튼으로 현재 지도 영역 기준 관광지 갱신
4. 우측 사이드바 즐겨찾기 탭에서 등록 목록 확인 및 경로 추가

### 시나리오 3: 저장된 여행 계획을 게시판에 공유

1. 경로 플래너에서 코스 구성 후 "계획 저장"
2. 게시판 → 자유 탭 → "+ 기록 쓰기" 클릭
3. 여행 계획 드롭다운에서 저장 계획 선택하여 글에 첨부
4. 다른 사용자가 게시글 상세 → "내 루트로 가져오기" 클릭하여 경로 플래너에 로드

### 시나리오 4: 비회원 챗봇 체험 → 회원가입 → JWT 인증

1. 비로그인 상태에서 챗봇 사용 (공개 엔드포인트 `/api/chatbot/ask`)
2. 계획 저장 시도 → 로그인 모달 유도
3. 회원가입 → 로그인 → JWT 토큰 발급 및 localStorage 저장
4. 새로고침 후에도 로그인 상태 유지 확인

### 시나리오 5: 관광지 키워드 검색 + Kakao fallback

1. 검색창에 "경복궁" 입력
2. 내부 DB에서 공식 관광공사 명칭으로 매칭 → 지도 마커 표시
3. DB 미등록 장소명 입력 시 Kakao Places API 자동 fallback 검색
4. 마커 팝업 → "경로에 추가" 클릭

### 시나리오 6: 이동 요금 비교 (대중교통 vs 택시)

1. 경로 플래너에 장소 3개 이상 추가
2. "대중교통" 탭 → ODsay API 기반 구간별 요금 및 총 대중교통 요금 확인
3. "자동차" 탭 → 카카오 모빌리티 API 기반 예상 택시비 확인
4. 두 요금 비교 후 이동 수단 선택

### 추천 질문 예시

| 상황 | 질문 예시 |
|------|---------|
| 데이트 코스 | "서울 주말 데이트 코스 추천해줘" |
| 당일치기 | "부산 당일치기 여행 코스 추천해줘" |
| 날씨 맞춤 | "비오는 날 가기 좋은 실내 관광지 추천해줘" |
| 예산 제한 | "예산 5만원으로 갈 만한 코스 추천해줘" |
| 야간 관광 | "서울 야경 명소 추천해줘" |
| 가족 여행 | "아이와 함께 가기 좋은 서울 체험 관광지 알려줘" |
